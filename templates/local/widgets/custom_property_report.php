...
<?php if(config_item('report_property_enabled') == TRUE && isset($property_id) && isset($agent_id)): ?>
    ...
    <?php if(!is_array($this->session->userdata('reported')) || !in_array($property_id, $this->session->userdata('reported'))): ?>
        <a class="btn btn-custom-s btn-custom-default popup-with-form-report" id="report_property" href="#popup_report_property" style=""><i class="icon-flag"></i> <?php echo lang_check('Report property'); ?> <i class="load-indicator"></i></a>
    <?php endif; ?>    
