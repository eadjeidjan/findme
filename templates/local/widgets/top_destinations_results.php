    ...
    if (isset($tree_listings[$val->id]) && count($tree_listings[$val->id]) > 0)
        foreach ($tree_listings[$val->id] as $key => $_child) {
            ...
            if(!empty($options->{'body'}))
            {
                ...
                $href = slug_url($lang_code.'/'.config_item("results_page_id").'/?search={"v_search_option_'.$treefield_id.'":"'.rawurlencode($treefield['title_chlimit'].' - '.$child['title'].' - ').'"}');
                $child['url'] = $href;
            }
        ...
        <div class="row result-container row-flex">
            ...
            <div class="col-md-4 col-sm-6">
                <div class="card card-destinations">
                    <img src="<?php _che($item['image_url']);?>" class="preview" alt="" />
                    <div class="title">
            ...
            <?php if($i==count($treefields))echo '</div>';?>
            <?php $i++; endforeach; ?>
